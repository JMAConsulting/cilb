...
cv api4 Cilb.importAccounts cutOffDate=$CUT_OFF_DATE
$DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/1-accounts.sql

cv api4 Cilb.importCandidates cutOffDate=$CUT_OFF_DATE
$DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/2-candidate-data.sql

cv api4 Cilb.importCandidateEntities cutOffDate=$CUT_OFF_DATE
$DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/3-candidate-entities.sql

cv api4 Cilb.importExams cutOffDate=$CUT_OFF_DATE
$DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/4-exams.sql
...
for YEAR in 2019 2020 2021 2022 2023 2024 2025; do
  ...
  $DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/5-$YEAR-1-registrations.sql

  cv api4 Cilb.importRegistrationsBF cutOffDate=$CUT_OFF_DATE transactionYear=$YEAR
  $DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/5-$YEAR-2-registrations-bf.sql

  cv api4 Cilb.importActivities cutOffDate=$CUT_OFF_DATE transactionYear=$YEAR
  $DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/5-$YEAR-3-activities.sql

done

cv api4 Cilb.importBlockedUsers cutOffDate=$CUT_OFF_DATE
$DB_DUMP > $BACKUP_FOLDER/$MIGRATION_NAME/6-blocked-users.sql
